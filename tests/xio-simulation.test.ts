...
test('bootstrap supports all eight registered roles', () => {
  const roles = ['legal', 'sales', 'marketing', 'engineering', 'support', 'finance', 'product', 'research'];
  const sim = new Simulation({ roles, agentsPerRole: 2, cycles: 0 });
  sim.bootstrap();
  const audit = sim.exportAudit();
  assert.equal(audit.finalPopulation.length, 16);
